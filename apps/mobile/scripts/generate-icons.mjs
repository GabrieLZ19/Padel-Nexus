...
const brandSvg = path.join(root, "assets/brand/LogoAccessory.svg");
...
async function main() {
  ...
  const logoSvg = await readFile(brandSvg);

  const icon = await renderPng(logoSvg, 1024);
  const adaptive = await renderPng(logoSvg, 1024);
  const splash = await renderPng(logoSvg, 512);
  const favicon = await renderPng(logoSvg, 48);
  const notification = await renderPng(Buffer.from(NOTIFICATION_SVG), 96);
  ...
  console.log("Iconos generados con fondo transparente en", outDir);
}
